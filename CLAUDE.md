...
- **The progress split is measured, not assumed.** `Pacer` in `commands.rs` weighs timed decode cost against how fast the model transcribed last time, reads/writes `<data_dir>/engine_speeds.json` (audio seconds per wall-clock second, blended 60/40 so one throttled run doesn't swing it), and falls back to `DEFAULT_DECODE_SHARE` only on a model's first run. The bar is clamped monotonic — a refining estimate that walked backwards reads as lost ground. Two separate thresholds: the realtime multiplier shown in the UI needs a 0.5–1 s sample so it doesn't flicker, the split needs only 0.05 s, because a short file's whole decode can be over before the display threshold is met. `TranscriptionProgress` carries `speed` for the UI's `12×` badge. All four local engines share `local_progress_sink`; Parakeet+Sherpa passes one sink to both `audio_to_pcm_with_progress` and `transcribe_pcm` so a single `Pacer` spans both stages.
