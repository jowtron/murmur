...
/// Hybrid engine: transcribe locally with Whisper (better text), diarize with
/// AssemblyAI (better speaker turns), then overlay the two. Output is Whisper's
/// words grouped by AssemblyAI's speakers — best of both. Runs both a local
/// Whisper pass and a billed AssemblyAI pass.
#[tauri::command]
pub async fn transcribe_whisper_aai(
    job: WhisperAaiJob,
    window: Window,
    state: State<'_, AppState>,
) -> Result<AssemblyAIResult, String> {
    ...
    let model = parse_model(&job.model);
    if !transcriber::is_model_downloaded(&model) {
        return Err(format!("Whisper model '{}' is not downloaded.", job.model));
    }
    ...
    // ---- Stage 1: Whisper transcription (local), reusing an existing SRT if present ----
    emit_aai(&window, &job_id, &file_name, 0.02, "loading_model");
    let existing_srt = output_dir.join(format!("{}_transcription_{}.srt", stem, job.model));
    let whisper_segments: Vec<(f64, f64, String)>;
    let whisper_duration: f64;
    ...
    if existing_srt.exists() {
        emit_aai(&window, &job_id, &file_name, 0.45, "reusing existing whisper srt");
        let content = std::fs::read_to_string(&existing_srt)
            .map_err(|e| format!("Failed to read existing SRT: {}", e))?;
        whisper_segments = sherpa::parse_srt(&content);
        if whisper_segments.is_empty() {
        ...
        whisper_duration = whisper_segments.iter().map(|(_, e, _)| *e).fold(0.0_f64, f64::max);
    } else {
        let win = window.clone();
        let jid = job_id.clone();
        let fname = file_name.clone();
    ...
    // ---- Stage 2: AssemblyAI diarization (cloud) ----
    let mut trim_map: Vec<silence_trim::SegmentMap> = Vec::new();
    let mut temp_upload_path: Option<PathBuf> = None;
    let path_for_upload = if job.trim_silence {
        emit_aai(&window, &job_id, &file_name, 0.47, "trimming silence");
        let src = audio_path.clone();
        let thr = job.silence_threshold_db;
        let min_sil = job.min_silence_secs;
        let pad = job.silence_padding_secs;
        ...
        emit_aai(&window, &job_id, &file_name, 0.47, "extracting audio");
        let src = audio_path.clone();
        let wav = tokio::task::spawn_blocking(move || {
            silence_trim::extract_audio_to_temp_wav(&src)
        })
    ...
    emit_aai(&window, &job_id, &file_name, 0.50, "uploading");
    let api_key = job.api_key.clone();
    let path_for_upload_clone = path_for_upload.clone();
    let upload_url = tokio::task::spawn_blocking(move || {
        assemblyai::upload_file(&path_for_upload_clone, &api_key)
    ...
    emit_aai(&window, &job_id, &file_name, 0.55, "submitting");
    let api_key_2 = job.api_key.clone();
    let lang = job.language_code.clone();
    let models = job.speech_models.clone();
    let speakers_expected = job.speakers_expected;
    ...
    emit_aai(&window, &job_id, &file_name, 0.60, "diarizing");
    let mut elapsed_polls: u32 = 0;
    let mut final_transcript = loop {
        bail_if_cancelled!();
        let t = assemblyai::poll(&transcript_id, &job.api_key).await?;
        match t.status.as_str() {
            ...
            _ => {
                elapsed_polls += 1;
                let est = (0.60 + (elapsed_polls as f32 * 0.01)).min(0.92);
                emit_aai(&window, &job_id, &file_name, est, "diarizing");
                tokio::time::sleep(assemblyai::poll_interval()).await;
            }
        }
    };

    // Remap AAI times back to the original timeline if silence was trimmed, so
    // they align with the (untrimmed) Whisper segments.
    if !trim_map.is_empty() {
        if let Some(utts) = final_transcript.utterances.as_mut() {
            for u in utts.iter_mut() {
                u.start = silence_trim::map_trimmed_ms_to_original_ms(u.start, &trim_map);
    ...
    // ---- Stage 3: overlay Whisper text onto AAI speaker turns ----
    emit_aai(&window, &job_id, &file_name, 0.95, "merging");
    let merged = assemblyai::assign_whisper_to_utterances(&whisper_segments, &utterances);

    let srt_path = output_dir.join(format!("{}.diarized.whisper-aai.srt", stem));
    let txt_path = output_dir.join(format!("{}.diarized.whisper-aai.txt", stem));
    let json_path = output_dir.join(format!("{}.diarized.whisper-aai.json", stem));

    std::fs::write(&srt_path, sherpa::segments_to_srt(&merged))
        .map_err(|e| format!("Failed to write SRT: {}", e))?;
    std::fs::write(&txt_path, sherpa::segments_to_text(&merged))
        .map_err(|e| format!("Failed to write TXT: {}", e))?;
    let json_value = assemblyai::merged_to_json(&merged, whisper_duration, &job.model);
    std::fs::write(&json_path, serde_json::to_string_pretty(&json_value).unwrap_or_default())
        .map_err(|e| format!("Failed to write JSON: {}", e))?;
